    ...
    const fetchLogs = async () => {
      try {
        if (!import.meta.env.VITE_AWS_LAMBDA_FUNCTION_NAME) {
          throw new Error("Lambda function name is missing in environment variables.");
        }
        const lambda = new AWS.Lambda({
          region: import.meta.env.VITE_AWS_REGION || "ap-south-1",
          accessKeyId: import.meta.env.VITE_AWS_ACCESS_KEY_ID,
        ...
        const params = {
          ...
          Payload: JSON.stringify({
            body: JSON.stringify({ prompt: "Fetch last 5 logs from CloudWatch" })  // Wrap in 'body'
          }),
        };
    ...
    try {
      ...
      try {
        ...
        if (typeof data.body === "string") {
          data = JSON.parse(data.body); // ✅ Parse `body` if it's a string
        }
      } catch (jsonError) {
        throw new Error("❌ Invalid JSON response from Lambda.");
      }
      ...
      if (data && data.response) {
        ...
        setResponse("⚠️ No valid response received from Lambda.");
      }
    } catch (error) {
